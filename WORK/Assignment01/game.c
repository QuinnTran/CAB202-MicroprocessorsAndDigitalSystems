...
void display_border()
{
    ...
    draw_formatted(23, 3, ":%d",(game_time % 60));
...
void end_game()
{
    ...
    int game_time = (int) get_current_time() - time_start;
    clear_screen();
    draw_formatted(( screen_width() / 2)-10, ( screen_height()/ 2)-2,"Game over! Thank you for playing!");
    draw_formatted(( screen_width() / 2)-10, ( screen_height()/ 2), "Time: %d",(game_time/60));
    draw_formatted(( screen_width() / 2) -2, ( screen_height()/ 2), ":%d",(game_time % 60));
    draw_formatted(( screen_width() / 2)-10, ( screen_height()/ 2)+2, "Score: %d", score);
    draw_formatted(( screen_width() / 2)-10, ( screen_height()/ 2)+4,"Press Q to exit.");
...
void draw_all() {
    ...
    draw_sprites(good_block, (80*columns())/100);
    draw_sprites(bad_block, (20*columns())/100);
    sprite_draw(border);
    display_border();
    show_screen();
}
...
void setup(void)
{
    player = sprite_create((screen_width() - PLAYER_WIDTH) - 4, (screen_height() - PLAYER_HEIGHT) - 47, PLAYER_WIDTH, PLAYER_HEIGHT, player_image);
    starting_block = sprite_create((screen_width() - BLOCK_WIDTH) - 2, (screen_height() - BLOCK_HEIGHT) - 45, BLOCK_WIDTH, BLOCK_HEIGHT, good_block_image);
...
void process( void )
{
    ...
    // block is not overlap with another block
    for (int i = 0; i < 16;i++){
        if (sprites_collide_any(good_block[i], bad_block, 5)){
            setup_blocks();
        }
    }
    ...
    if ( sprites_collide_any( player, good_block, (80*columns())/100 ) )
    {
        sprite_move(player, 0, -1);
        y_speed = 0;
        score += 1;
    ...
    if(sprites_collide_any(player, bad_block, (20*columns())/100))
    {
        sprite_move(player, 0, -1);
        lives -= 1;
        setup();
