...
// The game_paused information displayed is:
void game_pauses(){
    clear_screen();
...
// int border_collision(Sprite *target){
//     int collide = 0;
...
// Always at least player sprite height + 2 pixels vertically separated from other blocks.
// There are at least 7 safe blocks on the screen at one time.
// There are at least 2 forbidden blocks on the screen at one time.
// Safe and forbidden blocks are easily distinguished from each other.
// Blocks have no consistent observable pattern and each block:
// Appears in a randomly selected row,
// Appears in a randomly selected column,
...
void process(void){
    ...
    // block is not overlap with another block (assignment 1)
    for (int i = 0; i < num_gblock;i++){
