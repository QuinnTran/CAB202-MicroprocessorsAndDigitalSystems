...
int onland(Sprite s){
    ...
    if((ypos + ysize >= starting_block.y - 2)
    && (ypos  <= starting_block.y + 2 )
    && (xpos + xsize -1 >= starting_block.x)
    && (xpos <= starting_block.x + starting_block.width-1)
    ){
        return 1;
    }

    for(int i = 0; i < num_gblock; i++){
        if((s.y + s.height >= goodBlock[i].y - 0.5) 
        && (s.y + s.height <= goodBlock[i].y + 0.49 )
        && (s.x + s.width -1 >= goodBlock[i].x)
        && (s.x <= goodBlock[i].x + goodBlock[i].width - 1)){
            return 1;
        }
    }
    return 0;
...
void game_setup(){
    sprite_init(&player,5, 2,PLAYER_W, PLAYER_H, player_jpg);//display player
    sprite_init(&starting_block, 5, 2+PLAYER_H, BLOCK_W, BLOCK_H, good_block);//display starting block
