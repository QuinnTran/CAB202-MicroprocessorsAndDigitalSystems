...
void setup(void) {
    ...
    //      initial location which is displaced 9 units to the left and 
    //      7 units above the star.
    char * station_image = "9";
    station = sprite_create(x_star - 9,y_star -7, 1, 1, station_image);
    star = sprite_create(x_star,y_star,1,1,"*");
    ...
    double dx = sprite_x(star) - sprite_x(station);
    ...
    //      star and that of the station.
    double dy = sprite_y(star) - sprite_y(station);
    ...
    //      The distance can be calculated using dx ands dy, and applying the 
